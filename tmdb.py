...
for i in genres_list:
    ...
    for d in i:
        r.append(d['name'])
    result.append(r)
df = df.assign(genres_list=result)

df_genres = pd.DataFrame.from_records(flat_list).drop_duplicates()

df_columns = ['budget', 'id', 'imdb_id', "original_title", "revenue", "runtime"]
...
df[df_columns].to_csv("tmdb_movies", index=False)
df_genres.to_csv("tmdb_genres", index=False)
